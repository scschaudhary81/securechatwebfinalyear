//packages
import 'package:final_year_project/modals/chat_message.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget _messagesList() {
    ...
    return Expanded(
      child: (){
        if(_chats!=null) {
          if(_chats.isEmpty){
            return const Center(child: Text("No Chats Found",style: TextStyle(color:Colors.teal),),);
          }else{
            return ListView.builder(itemCount:_chats.length,itemBuilder: (ctx,idx){
  ...
  Widget _messageTile(Chat _chat) {
    ...
    bool _isActive = _recepients.any((_chatUser) => _chatUser.wasRecentlyActive());
    String _subTitle = "No Messages";
    if(_chat.messages.isNotEmpty) {
      _subTitle = _chat.messages.first.type ==MessageType.TEXT?_chat.messages.first.content:"Image File";
    }
